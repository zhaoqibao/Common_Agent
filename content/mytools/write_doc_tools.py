...
def convert_file(filepath, output_format):
  ...
  :return 输出文件路径
  """
  # 1. 得到输入文件的格式（用 splitext 只取最后的扩展名，避免路径中间带点时出错）
  input_format = os.path.splitext(filepath)[1].lstrip('.')
  # 2. 根据输出文件的格式，拼出输出文件的路径
  output_file_path = os.path.splitext(filepath)[0] + '.' + output_format
  # 3. 再判断是否是pdf转换
...
if __name__ == '__main__':
  filepath = r'D:\agent_files\01a0476b-354c-7b60-87c8-3b253efc8ae6\user_upload\it产品维修指南.md'
  convert_file(filepath, 'docx')
